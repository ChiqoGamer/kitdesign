...
/** Editor de una zona: patrón, colores y ajustes. Reutilizable. */
export function ZoneControls({ zone }: { zone: ZoneId }) {
      ...
      <PanelSection title="Patrón">
        <div className="grid grid-cols-3 gap-2">
          {(Object.keys(PATTERN_LABELS) as PatternId[]).map((p) => (
            <button
              key={p}
              onClick={() => dispatch({ type: "SET_ZONE_PATTERN", zone, pattern: p })}
              title={PATTERN_LABELS[p]}
              className={`group overflow-hidden rounded-lg border-2 transition-colors ${
                fill.pattern === p ? "border-accent" : "border-ink-700 hover:border-ink-500"
              }`}
            >
              <div className="h-11 w-full" style={patternPreviewStyle(p, resolved)} />
      ...
      <PanelSection title="Colores">
        {Array.from({ length: colorCount }).map((_, slot) => (
          <div key={slot} className="mb-4 last:mb-0">
            <div className="mb-1.5 text-xs text-ink-300">{SLOT_LABELS[slot]}</div>
              ...
              {design.palette.map((entry, i) => {
                ...
                return (
                  <button
                    key={entry.id}
                    onClick={() =>
                      dispatch({ type: "SET_ZONE_COLOR", zone, slot: slot as 0 | 1 | 2, color: ref })
                    }
                    title={entry.name}
          ...
          {paramKeys.map((key) => {
            ...
            return (
              <Slider
                ...
                format={(v) => (range.step < 1 ? v.toFixed(2) : String(Math.round(v)))}
                onChange={(value) =>
                  dispatch({ type: "SET_ZONE_PARAM", zone, key: key as keyof PatternParams, value })
                }
              />
            );
          })}
